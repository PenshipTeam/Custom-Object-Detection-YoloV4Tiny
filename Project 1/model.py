...
while True:
    ...
    # Iterate over the zip of classes, scores, and boxes
    for (classid, score, box) in zip(classes, scores, boxes):
        # Check if classid is a scalar value (single detection)
        if isinstance(classid, int):
            ...
            for cid, scr, bx in zip(classid, score, box):
                color = COLORS[cid % len(COLORS)]
                label = "%s : %f" % (class_name[cid], scr)
                cv.rectangle(frame, bx, color, 1)
                cv.putText(frame, label, (bx[0], bx[1]-10), cv.FONT_HERSHEY_COMPLEX, 0.3, color, 1)
